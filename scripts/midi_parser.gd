...
func parse_file(path: String, difficulty: String = "Expert") -> bool:
	...
	file.close()
	return parse_data(data, difficulty)

func parse_data(data: PackedByteArray, difficulty: String = "Expert") -> bool:
	if data.size() < 14:
	...
	# --- Find note track ---
	var note_track = null
	var track_priority := ["PART GUITAR", "PART BASS", "PART KEYS"]
	...
	print("MidiParser: using track '%s'" % note_track["name"])
...
static func scan_difficulties_from_data(data: PackedByteArray) -> Array[String]:
	# For MIDI files, all 4 difficulties are typically available
	# A more thorough check would parse and look for notes in each range
	var parser := MidiParser.new()
	if not parser.parse_data(data, "Expert"):
		return []

	var result: Array[String] = []
	# Re-parse note track to check which difficulties have notes
	# For now, return all standard difficulties
...
func _extract_lyrics(track: Dictionary) -> void:
	var raw_lyrics: Array = []  # [{tick, text}]
	for ev in track["events"]:
	...
	# Build phrases — group lyrics into phrases separated by gaps > 2 seconds
	var phrase_syllables: Array = []
	var phrase_gap_ms := 2000.0

	for i in range(raw_lyrics.size()):
		var lyr = raw_lyrics[i]
