...
func _ready() -> void:
	...
	_ui_scale = 1.0
	song_source = "Fleetwood Mac - The Chain.chart"
	song_instrument = "guitar"
	song_difficulty = "Expert"
	song_preset = "Tiles"
	_build_loading_screen()
	is_loading = false
	loading_status_label.text = I18n.t("loading_stage").to_upper()
	_show_loading_countdown(3)
...
func _process(_delta: float) -> bool:
	...
	if image == null or image.is_empty():
		push_error("Loading UI capture viewport image is unavailable")
		quit(1)
		return true
	assert(image.get_size() == CAPTURE_SIZE)
	DirAccess.make_dir_recursive_absolute(
		ProjectSettings.globalize_path("res://tmp"))
	var output_path := "res://tmp/loading-ui-landscape.png"
	assert(image.save_png(output_path) == OK)
	print("LOADING UI CAPTURE PASS: ", output_path, " ", image.get_size())
	quit(0)
	return true
